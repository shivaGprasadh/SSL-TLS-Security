...
def scan_results():
    ...
    # Apply filters
    if domain_filter:
        query = query.filter(Domain.hostname == domain_filter)
    ...
    # Get all domains that have scan results for the filter dropdown
    available_domains = db.session.query(Domain).join(ScanResult).filter(
        Domain.active == True
    ).distinct().order_by(Domain.hostname).all()
